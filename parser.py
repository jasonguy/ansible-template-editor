...
def get_ansible_filters():
    ...
    ansible_filters = {}
    for name,value in getmembers(core):
        if not name.startswith('_') and isfunction(value):
            ansible_filters[name] = value

    for name,value in getmembers(ipaddr):
        if not name.startswith('_') and isfunction(value):
            ansible_filters[name] = value

    for name,value in getmembers(mathstuff):
        if not name.startswith('_') and isfunction(value):
            ansible_filters[name] = value
...
def convert():
    ...
    app.logger.debug('Add the following ansible filters to Jinja environment: %s' % ', '.join(ansible_filters.keys()))
    jinja2_env.filters.update(ansible_filters)
